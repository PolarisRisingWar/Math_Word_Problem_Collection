...
第四阶段-测试verifier：`CUDA_VISIBLE_DEVICES=4 python codes/finetune/gpt2/test_w_calculator.py -dn GSM8K -ds datasets/gsm8k -cp my_checkpoints/gpt2verifier_gsm8k -rt codes/results/verifier_GSM8K_w_calculator_result.txt`（我也不知道具体跑了多久，因为跑了一半服务器自动重启了，我断点续跑的）（其实不应该直接推理的，应该先推理100倍样本然后再打分。我当时没这么写是因为我忘了还有这茬……总之如果大家需要精确复现的话记得改改这部分）